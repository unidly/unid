...
A systematic approach to naming server and other hardware is required.

## Server A record name

Each server has a distict A record name that is a single word subdomain drawn
from a [list of words](https://web.archive.org/web/20090918202746/http://tothink.com/mnemonic/wordlist.html)
from Oren Tirosh’s mnemonic encoding project. These 1633 words were chosen very
specifically to be short (4-7 letters), phonetically different from one another,
easy to understand over the phone, and also recognizable internationally.

Example:

'''
banjo.unidly.io.    A    192.0.2.11
'''

## Server CNAME record name

CNAME (Canonical Name) record functions as an alias rather than pointing to a
specific IP address. It maps on domain name (the alias) to anothe "true" or 
canonical domain name.

'''
  web01.dev.ca.us.unidly.io.   CNAME    banjo.unidly.io
'''

Anatomy of a unid server CNAME:

* io - the top-level domain
* unidly - the domain
* us - geographic location country code ISO 3166-1
* ca - geographic location subdivision code ISO 3166-2 (2 or 3 characters)
* dev - environment
* web01- purpose and serial number

### Geographic location

These defines where the server is located using ISO 3166 country and subdivision
codes.

### Environment designations

This defines who the server belongs to.

* dev - Development (R&D)
* tst - Testing (R&D and QA)
* stg = Staging (QA and Operatons)
* prd - Production server (Operations)

### Purpose designations

This defines what the server is used for.
...
The serial number is a zero-padded two digit number ranging from  01 to 99.

## Special cases

Other equipment that is accessible via the internet provides specialized functionality.
This equipment is addressed using functional abbreviations for the DNS A record itself:
...
For assets that are owned and managed by Unidly, these are assigned domain names as
follows:

'''
rtr01.ca.us.unidly.io    A    192.42.0.19
'''

## A record subdomain wordlist
