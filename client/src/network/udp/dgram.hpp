/**
 * @file
 * @brief Header file for UDP datagram
 *
 * UDP software divides the data structures that store incoming datagrams
 * into two conceptual pieces: The first consists of queues for arriving
 * datagrams, while the second piece contains mapping information that UDP
 * uses to select a queue. The first piece is part of the interface between
 * UDP and the application program that needs to extract arriving datagrams.
 * The second piece is part of the operating system - UDP software uses it
 * to select a queue, but application programs cannot access it.
 *
 * dgram.hpp contains the declaration of the queues used by application
 * programs.
 *
